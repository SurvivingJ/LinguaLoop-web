...
open_questions:
  - "Chinese (language_id=1) and Japanese (language_id=3) seeds remain absent for all three tasks."
...
`services.prompt_service.get_template_config`. Only English (`language_id = 2`)
templates exist today — Chinese and Japanese are not yet seeded.

| Task | Active version | Default model | Migration |
|------|----------------|---------------|-----------|
| `vocab_prompt1_core` | v4 | `google/gemini-2.5-flash-lite` | `migrations/restrict_l5_and_lock_l8_sentence.sql` |
| `vocab_prompt2_exercises` | v2 | `anthropic/claude-sonnet-4-6` | `migrations/improve_vocab_pipeline_prompts.sql` |
| `vocab_prompt3_transforms` | v2 | `anthropic/claude-sonnet-4-6` | `migrations/improve_vocab_pipeline_prompts.sql` |
...
- **Stronger L1 distractor rules** (Levenshtein, first/last letter, homophone
  whitelist) and **L8 anti-substitution test** for collocation distractors.
...
## Prompt 2 v2 (active) — `vocab_prompt2_exercises`
...
Source: [migrations/vocabulary_ladder_schema.sql:200-241](../../migrations/vocabulary_ladder_schema.sql#L200-L241)
