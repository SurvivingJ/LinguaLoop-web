...
- **What shipped.** A `selection_tuning` settings table, the vocabulary term in
  ...
  replay harness.
- **Parity proven before applying.** At weight 0 the rows are identical in
  content and order for all 13 users × en/zh/ja. That branch *is* the old query,
  verbatim.
- **Two spec corrections.** The Zipf prior now gives 0.85 at `ability_zipf`,
  where it had given 0.5. The no-calibration fallback is the learner's own uvk
  85% crossing, not a median; it is the path that runs today, because
  `user_calibration_state` has 0 rows.
- **One deviation.** The tier ceiling demotes rather than excludes, so M5 holds.
- **Replay** of the 21 ja first attempts: the vocabulary arm puts 93% of served
  tests inside u*±tol against 77%, and the unknown share of the test actually
  taken correlates with its score at Spearman −0.59.
- **Other work.** TASK-751 has a design proposal and nothing reseeded. TASK-765
  (Calibration's migration) is confirmed live. See
  [[evaluations/selection-replay-2026-09-10]] and `wiki/log.md` 2026-09-10.) | Pages: 110

Prior: 2026-08-24 (**TASK-732 done — `build_daily_session` now guarantees a practice slice of every day.** A live-account audit found `practice_acquisition_min`/`practice_maintenance_min` were 0 on **every** `daily_test_loads` row ever written (both language plans, back to account creation) — the vocabulary ladder had never once been scheduled into `/session`. Root cause: the resolver's single greedy loop ranked test/surface and practice candidates in one shared pool by per-minute value; test candidates score 5-15× higher by construction (`skill_value/time_estimate` ~0.06-0.15 vs practice's flat `alpha×share` ~0.006-0.014), so practice — sorted last — was never reached once remaining weekly test-slot counts alone filled the day, which they always could (a separate, still-open sizing issue: `target_counts` and `practice_target_minutes` are sized independently against the same weekly ceiling in `study_plan_service.py` and never reconciled). Fix splits today's budget into a test/surface share and a practice share *before* ranking, proportional to each side's outstanding weekly minutes, then ranks within each side as before. Applied live; verified live — the test account's zh/ja plans both went from 0 to a real acquisition slot on the first post-fix call. See [[tasklist/master]] and `wiki/log.md` 2026-08-24) | Pages: 109
