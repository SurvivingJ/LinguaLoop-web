# ADAPTED FROM services/vocabulary_ladder/deterministic/phonology.py:1-451
#
# Copied VERBATIM (this module is pure string manipulation over a hardcoded
# confusion-set inventory - no DB, no network, no LLM, no other production
# imports) so the zero-LLM prototype's reading/tone distractor QUALITY is
# identical to production's, not a reimplementation that could silently
# diverge. Only this banner was added; every line below is the real logic.
"""
...
corpus instead — see ``lexicon.py`` in this prototype (adapted from
readings.py).
