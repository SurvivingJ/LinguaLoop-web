-- ============================================================================
-- Fix: get_practice_session — scope the Acquisition eligibility count to
-- the requested language.
-- Date: 2026-08-25
--
-- Bug: the "no eligible words" pre-check in the Acquisition path counted
-- user_word_ladder rows across ALL of a learner's languages, not just
-- p_language_id (user_word_ladder has no language of its own — see
-- services/vocabulary_ladder/speed_round.py::mastered_sense_ids for the
-- established fix pattern of joining through dim_word_senses ->
-- dim_vocabulary). A learner with an eligible word in one language (e.g.
-- Chinese) who opens Acquisition practice in a brand-new second language
-- (e.g. Japanese, with an empty ladder) got v_eligible_count > 0 from the
-- *other* language's row, so the function skipped the intended
-- no_content_reason = 'no_eligible_words' short-circuit, ran the
-- language-scoped word loop (correctly, and correctly empty), and fell
-- through to the generic 'all_complete' default — which misreports an
-- empty, never-started ladder as "nothing left to learn".
--
-- This supersedes only the get_practice_session body defined in
-- phase12_get_practice_session.sql; that file remains canonical for the
-- ladder_compute_priority helper, which is unchanged.
-- ============================================================================
...
BEGIN
    ...
    IF v_mode = 'acquisition' AND v_elapsed_seconds < v_target_seconds THEN
        ...
        -- FIX: scoped to p_language_id via dim_word_senses -> dim_vocabulary
        -- (user_word_ladder has no language of its own — a learner's ladder
        -- row in one language must not mask an empty ladder in another).
...
COMMENT ON FUNCTION public.get_practice_session IS
    ...
    'Eligibility counts (auto-mode active-ladder count and the Acquisition '
    'no-content check) are scoped to p_language_id via dim_word_senses -> '
    'dim_vocabulary, since user_word_ladder carries no language of its own.';
