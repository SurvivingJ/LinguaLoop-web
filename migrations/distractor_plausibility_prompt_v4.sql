-- Distractor-plausibility judge prompt v4 (zh/en/ja).
--
-- WHY: the v3 output spec used numeric keys as FIELD SELECTORS
--   {"1": [ratings], "2": [reasons]}
-- while the same prompt showed the model a NUMBERED distractor list and asked
-- for "one entry per distractor". The model resolved the ambiguity as
-- key=distractor-index, which left no slot for the number -- so it returned
-- reasons with NO ratings at all. schemas.py then fabricated a 3 for each
-- missing rating, 3 maps to 'flag', and 80% of live verdicts became
-- review-queue entries for judgments nobody made.
--
-- v4 changes ONLY the output-format block: numeric keys now index the
-- DISTRACTOR (the shape the model already drifts to unprompted), with the
-- rating pinned to array position 1:
--   {"1": [5, "reason"], "2": [3, "reason"], "3": [2, "reason"]}
-- The rubric, the 5-point scale and the worked example are carried over
-- verbatim from v3, so the change is isolated to the response shape.
-- Numeric keys only -- no named fields -- so zh/ja prompts stay free of
-- English field names.
--
-- Models are carried forward unchanged to isolate the prompt effect.
-- Reversible: UPDATE prompt_templates SET is_active = (version = 3) WHERE
...
VALUES (
    'test_distractor_plausibility', 1, 4, TRUE,
...
JSON 之外不要输出任何文字，不要使用 markdown 代码块。',
    'v4: numeric keys index the distractor; rating pinned to array position 1. Fixes the v3 shape ambiguity that made the model omit ratings entirely.'
)
...
INSERT INTO prompt_templates
    (task_name, language_id, version, is_active, model, provider,
     template_text, description)
VALUES (
    'test_distractor_plausibility', 2, 4, TRUE,
...
INSERT INTO prompt_templates
    (task_name, language_id, version, is_active, model, provider,
     template_text, description)
VALUES (
    'test_distractor_plausibility', 3, 4, TRUE,
    'qwen/qwen3.6-flash', 'openrouter',
    'あなたは読解問題の品質評価員です。
