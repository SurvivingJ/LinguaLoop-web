-- ============================================================================
-- Phase 14 — Generation Quality — test-side cold-start K-factor decay
-- Date: 2026-05-24
--
-- Replaces the constant test-side K=32 in process_test_submission with a
-- cohort-aware decay:
--   *  <20 prior attempts:  K = 48  (cold start — let early attempts move
--                                    the rating fast so a bad operator tier
--                                    guess corrects in 5-10 plays instead
--                                    of 50+)
--   *  20-49 prior attempts: K = 24  (settling)
--   *  >=50 prior attempts:  K = 16  (stable; resists single noisy attempts)
--
-- User-side K-factor is unchanged (still 32, dampened to 16 when
-- furigana_used). Decay is asymmetric on purpose: the user's rating is a
-- per-skill aggregate over many tests, so their cohort size is large
-- already; the test is what we're calibrating.
--
-- Other than the test K-factor block, this file is the verbatim live
-- definition of process_test_submission as of phase 14 (formerly
-- process_test_submission_v2.sql).
-- ============================================================================
...
BEGIN
  ...
  IF v_is_first_attempt THEN
    ...
    BEGIN
      ...
      -- User-side K: constant 32, dampened to 16 when furigana_used.
      v_user_k_factor := c_user_k_factor * CASE WHEN p_furigana_used
        THEN c_furigana_dampener ELSE 1.0 END;

      -- Test-side K: cohort-aware cold-start decay. Lets early plays
      -- correct a wrong operator tier guess quickly; tightens once the
      -- rating has been pinned by a meaningful number of attempts.
