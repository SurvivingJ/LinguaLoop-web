-- ============================================================================
-- Dual Translation - taxonomy v3 seed (TASK-616, Stage 4 localisation)
...
UPDATE public.dt_taxonomy_version SET is_active = false WHERE is_active AND version <> 3;
...
VALUES (
  ...
  "templates": {
    ...
    'TASK-616 dual-translation taxonomy v3 - JA L2 localised. Adds en-ja / zh-ja per-pair tables and richer particle (wa/ga) + keigo_register (teineigo/sonkeigo/kenjougo) glosses+templates on top of v2 (EN). keigo_register is tagged under the fidelity dimension via the rubric; the up-weight is in dt_rubric_version v2. Supersedes v2 (single active row).'
)
...
--   SELECT version FROM public.dt_taxonomy_version WHERE is_active;    -- expect 3
-- ============================================================================
