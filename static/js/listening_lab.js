...
(function () {
    ...
    const TIER_SPEEDS = [0.75, 0.90, 1.00, 1.15];
    ...
    function renderTemplate(id, container) {
        ...
        if (window.LinguaI18n && typeof LinguaI18n.applyTranslations === 'function') {
            try { LinguaI18n.applyTranslations(container); } catch (_) { /* ignore */ }
        }
    }
    ...
    async function resumeOrStart() {
        try {
            const resp = await authFetch(
                `/api/listening-lab/${encodeURIComponent(slug)}/start`,
    ...
    function onAudioPlay() {
        ...
        if (audio.currentTime < 0.5) {
            state.playCount += 1;
            const pc = $('#playCount');
            if (pc) pc.textContent = state.playCount === 1
                ? '1 play'
                : `${state.playCount} plays`;
        }
    }
    ...
    function renderQuestions() {
        ...
        container.innerHTML = state.questions.map((q, idx) => {
            ...
            const options = choices.map((opt, optIdx) => `
    ...
    function applySubmitResult(result) {
        ...
        if (result.passed) {
            ...
            feedback.textContent =
                `${result.score}/5 — next speed unlocked: ${TIER_SPEEDS[state.currentTier].toFixed(2)}×`;
            feedback.style.display = 'block';
            ...
            feedback.textContent =
                `${result.score}/5 — ${PASS_THRESHOLD} needed to advance. Fresh questions coming up.`;
            feedback.style.display = 'block';
    ...
    function showCompletionModal(result) {
        ...
        $('#completionMessage').textContent =
            (typeof eloAfter === 'number')
                ? `New listening_lab rating: ${eloAfter}`
                : '';
